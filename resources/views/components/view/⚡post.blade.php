    ...
    <flux:main container class="m-6 rounded-xl bg-zinc-50 dark:bg-zinc-900">
        <div class="space-y-6">
            ...
            <div class="space-y-4">
                ...
                <div class="max-w-md flex items-center gap-4">
                    <flux:input
                    ...
                    >
                        <flux:select.option value="">
                            All Categories</flux:select.option
                        >
                        @foreach ($categories as $cat)
                            <flux:select.option
                                value="{{ $cat->id }}"
                            >
                                {{ $cat->name }}</flux:select.option
                            >
                        @endforeach
                    </flux:select>
                </div>
            ...
            <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                @forelse ($posts as $post)
                    <flux:card class="overflow-hidden flex flex-col h-full">
                        ...
                        @if ($post->image)
                            <div
                                class="aspect-video overflow-hidden bg-zinc-100 dark:bg-zinc-800 rounded-lg"
                            >
                                <img
                                    src="{{ $post->image }}"
                                    alt="{{ $post->title }}"
                        ...
                        <div class="p-5 flex flex-col flex-1 space-y-3">
                            ...
                            <flux:text
                                class="text-sm text-zinc-600 dark:text-zinc-300 line-clamp-3"
                            >
                                {{ $post->content }}
                            </flux:text>
